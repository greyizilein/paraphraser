...
export function reconstructionPrompt(a:{protectedText:string;targetWords:number;register:Register;voiceSample?:string}){const voice=a.voiceSample?.trim()?`\nWRITER VOICE SAMPLE\nStudy this sample for transferable style only: sentence movement, density, directness, rhythm, preferred level of explanation, use of qualification, paragraph pacing and vocabulary range. Do not copy phrases, facts, examples, names or subject matter from it. Reproduce the writer's habits, not the sample's content.\n\n${a.voiceSample.trim()}\n`:"";return`Reconstruct the passage from its meaning rather than paraphrasing its sentences.
...
- Let the argument have hierarchy: some points can be brief, others developed. Do not make every sentence equally polished, equally long, equally explicit or rhetorically symmetrical.
...
- Vary sentence architecture because the thought demands it, not for random variation.
- Preserve the author's degree of certainty, scepticism, emphasis and restraint.
